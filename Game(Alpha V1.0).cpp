...
const string VERSION = "Alpha V1.0";
string name;
int hp = 100;
int mp = 200;
int goldcoin = 500;
...
int fightos(string fenemy)
{
	...
	while(hp > 0)
	{
		...
		{
			cout<<"1.攻击 2.防御 3.背包 4.探察 0.逃跑"<<endl;
			cout<<"按下相应的按键选择 > ";
			xd = getch();
			if(xd == '1')
			{
			...
			if(xd == '2')
			{
				if(fenemy == xiaoguaiwu.name)
				{
					int didangshanghai = xiaoguaiwu.shanghai-mianshang;
					if(didangshanghai > 0)
					{
						cout<<endl<<"你进行防御，抵挡了"<<mianshang<<"点伤害。"<<endl;
						hp -= didangshanghai;
						Sleep(1000);
					}
					else
					{
						cout<<endl<<"你进行防御，抵挡了全部伤害。"<<endl;
						Sleep(1000); 
					}
				}
				if(fenemy == daguaiwu.name)
				{
					int didangshanghai = daguaiwu.shanghai-mianshang;
					if(didangshanghai > 0)
					{
						cout<<endl<<"你进行防御，抵挡了"<<mianshang<<"点伤害。"<<endl;
						hp -= didangshanghai;
...
int main()
{
	...
	while(true)
	{
		...
		cout<<"1.武装店 2.药房 3.旅馆 4.外出 5.属性 6.关于 0.退出"<<endl<<endl;
		cout<<"按下相应的按键选择 > ";
		xd = getch();
		if(xd == '1')
		{
